...
const PAGE_SIZE = 6;
const TABLE_HEADERS = ["Product Name", "Revenue", "Sales", "Reviews", "Active"];

export default function ProductListCard({ products, total }: { products: Product[]; total: number }) {
  ...
  const { page, totalPages, paged, setPage, nextPage, prevPage } = usePagination(filtered(), PAGE_SIZE);
    ...
    <Card className="rounded-4 h-100 pt-1 border">
      <Card.Body className="p-2 px-4">
        ...
        <div style={{ overflowX: "auto", fontSize: "0.95rem" }}>
          <div className={`${tableStyles.listGrid} px-2 py-2 small text-muted fw-semibold`}>
